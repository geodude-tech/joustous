...
- `src/engine/` is **pure logic, no DOM** — `applyMove(state, move) -> state`. Every rule change goes here with a unit test first (project is TDD).
  ...
  - `board.ts` — `BOARD_SIZES` per pack: `basic` is a **walled 2×2** (2 gems), others are 3×3 (3 gems). Engine derives board size from `board.length`, not a global constant. On a walled board (`state.walls`) a push that would shove a card off the edge is **illegal** rather than removing the card — keeps the tiny board from being a card sink
